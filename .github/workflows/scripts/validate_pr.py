...
def validate_branch_and_pr_title(branch, pr_title):
    """
    Validates if the branch name and PR title follow the 'ATOM-HW-' pattern.
    Args:
        branch (str): The name of the branch to validate.
        pr_title (str): The title of the pull request (PR) to validate.
    Returns:
        bool: True if both the branch and the pull request follow the 'ATOM-HW-' pattern, False otherwise.
    Prints:
        str: A message indicating whether the branch and PR title are valid or specifying the validation error.
    """

    # Verify if the branch starts with 'ATOM-HW-'
    if not branch.startswith("ATOM-HW-"):
        print(f"The branch '{branch}' does not start with the pattern 'ATOM-HW-'.")
        return False
    # Verify if the branch contains with 'ATOM-HW-'
    if "ATOM-HW-" not in pr_title:
        print(f"The PR title '{pr_title}' does not contain the pattern 'ATOM-HW-'.")
        return False
    
    print(f"Validation successful: The branch '{branch}' and the PR title '{pr_title}' follow the 'ATOM-HW-' pattern.")
